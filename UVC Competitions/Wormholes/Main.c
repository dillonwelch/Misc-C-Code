...
int main()
{
	...
	while(numOfCases > 0)
	{
		...
		wormholes = (Wormhole*) malloc(sizeof(Wormhole) * numOfWormholes); // Array of wormholes
		...
		d = (int*) malloc(sizeof(int) * numOfSystems); // D[x] distance array for each star system
		//int possible = 0;	// Whether or not it is possible to go back in time
		possible = 0;

		for(i = 0; i < numOfSystems; ++i) // Set the distances for each node to infinity
		{
			d[i] = INF;
		}
